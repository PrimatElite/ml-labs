...
- [Intelligent Checker](https://docs.google.com/document/d/1Tmn7BLnHXiAsdCpB_GQVDqUx3AoGEdfOoTyn3Eg3P7A/edit#)
...
Для полного ознакомления с форматом ограничений рекомендуется ознакомиться со схемой `restrictions_schema.json`
...
3. Для работы с конфигами рекомендуется использовать функции из модуля `src.util` (реализованы в файле `src/util/__init.py`)
...
| снято на фоне чистого белого листа формата а4               | предметы не должны перекрывать лист с многоугольником        |
...
- ограничение на минимум/максимум линейного размера объектов
 - объекты должны располагаться на расстоянии друг от друга, а также от края листа (диапазон допустимого расстояния)
...
Входная точка приложения - `test.py`. Код чекера лежит в папке `src/checker`.
