...
### Как загрузить пользовательский конфиг в своем чекере

Вместе с плейсером чекеру предоставляется конфигурационный файл,
удовлетворяющий схеме для ограничений в формате `yaml`.
...
| 8    | rotation                     | градусы                           | Повороты объектов относительно оси Z (объект стается в том же ракурсе) |
